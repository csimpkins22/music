#!/usr/bin/env python3
"""
Song 1 v6: "Dial Tone Lullaby"
Key: C major | Tempo: 72 BPM | 4/4 | ~4:30

v6 songwriter notes:
Building on v5's arrangement. This version addresses three production-level issues:

1. CHORUS 2 TEXTURE (bars 48-55): Drop synth lead on bars 50-51 entirely
   (no notes). Too many layers — let pad/organ dialogue alone. Lead returns bar 52.
2. BRIDGE DYNAMIC DIP (bars 62-63): Piano velocity drops to vel=35 on bars 62-63
   (was ~43). This breath before the final chorus makes the crash on bar 66
   more powerful.
3. VERSE 2 BASS THINNING (bars 38-39): When synth pad drops bars 38-39 for
   breathing room, bass ALSO thins — switch to whole-note roots instead of
   active pattern. Both return to full energy bar 40. True textural contrast.
"""
...
# FIX #3 (v6): Bass ALSO thins on bars 38-39 (i=6,7) — whole-note roots
#   instead of active pattern. Returns to full energy bar 40 with pad.
...
V2_BASS_THIN_BARS = {6, 7} # FIX #3 (v6): bass thins to whole-note roots (absolute 38-39)
...
for i in range(16):
    ...
    # FIX #3 (v6): Bass thins to whole-note roots on bars 38-39 (i=6,7)
    if i in V2_BASS_THIN_BARS:
        bass(bt(b + i), bass_notes[ci], vel=42, bar_num=i, pattern='whole')
    else:
...
# FIX #1 (v6): Drop synth lead on bars 50-51 (i=2,3). Too many layers.
#   Let pad/organ dialogue alone. Lead returns bar 52 (i=4).
CH2_LEAD_DROP_BARS = {2, 3}  # relative bars where lead is dropped (absolute 50-51)
...
# FIX #1 (v6): Phrases are indexed by pi (0-3), each covering 2 bars.
#   pi=0 -> bars 48-49, pi=1 -> bars 50-51, pi=2 -> bars 52-53, pi=3 -> bars 54-55
#   Drop pi=1 (bars 50-51) entirely. Lead returns pi=2 (bar 52).
lead_ch2 = [
...
for pi, phrase in enumerate(lead_ch2):
    # FIX #1 (v6): Skip pi=1 (bars 50-51) — no synth lead, let pad/organ breathe
    if pi == 1:
        continue
...
# FIX #2 (v6): Piano velocity drops to vel=35 on bars 62-63 (bridge bars i=6,7).
#   Dynamic dip / breath before the final chorus crash on bar 66.
BRIDGE_PIANO_DIP_BARS = {6, 7}  # relative bars where piano vel drops (absolute 62-63)

for i in range(8):
    ci = (i // 2) % 4
    voicing = gv(BRIDGE_CHORDS, ci, i)

    # FIX #2 (v6): Piano velocity dip on bars 62-63 (i=6,7)
    bridge_piano_vel = 35 if i in BRIDGE_PIANO_DIP_BARS else 43
    ...
    if ci == 0:  # Dm: top note A4, walks to Ab4
        piano_bridge_chromatic(bt(b + i), voicing, N('A',4), N('Ab',4),
                                vel=bridge_piano_vel, bar_num=i)
...
E.note(0, N('E',5), 22, bt(b + 10), BAR4 * 2 - 20, time_amount=15, vel_amount=3)
...
build_midi(E, 'Dial Tone Lullaby', bpm=72, total_bars=88,
           channel_names={
               0: 'Piano', 1: 'Synth Pad', 2: 'Synth Lead', 3: 'Clean Guitar',
               4: 'Distorted Guitar', 5: 'Organ', 6: 'Bass', 7: 'Strings',
               8: 'Vocal Melody', 9: 'Drums',
           },
           output_path='/home/user/music/compositions/v7/01_dial_tone_lullaby.mid')
