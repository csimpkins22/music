#!/usr/bin/env python3
"""
Song 3 v5: "Beautiful Machines"
Key: Eb major | Tempo: 84 BPM | 4/4 | ~124 bars

v5 handcrafted changes — 4 specific improvements over v4:

1. VERSE 1 MELODY PHRASE REWRITE (bars 20-21): Replaced stepwise motion
   with E5->G5->B5(leap up a 4th)->A5(fall)->F5->D5. Intervallic surprise
   injects chromatic tension into the otherwise diatonic verse.

2. CHORUS 1 BASS Db BAR CHROMATIC ASCENT: Instead of plain half notes,
   Db bar now walks chromatically Db->Eb->E->F (one per beat),
   earning the borrowed chord by climbing into it.

3. VERSE 3 TEXTURAL DROP (bars 64-67): Synth arpeggio removed for first
   4 bars of this passage. Piano/bass/vocals carry alone. Synth returns
   bars 68-69 with velocity spike (vel=45 vs original 30).

4. CRESCENDO SYNCOPATED SNARE (bars 98-101): Added snare hits on the
   "and" of beat 2 in bars 98 and 100. Creates disorientation matching
   the Cb chord strangeness.
...
for i in range(16):
    ...
    # v5 FIX #4: Syncopated snare hits on bars 98 and 100 (i=8 and i=10)
    # Snare on "and" of beat 2 — creates disorientation against the Cb chord
    if i == 8 or i == 10:
        snare_midi = 38
...
build_midi(E, 'Beautiful Machines', bpm=84, total_bars=124,
           output_path='/home/user/music/compositions/v5/03_beautiful_machines.mid')
