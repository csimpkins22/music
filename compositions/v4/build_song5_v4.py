...
v4 changes (handcrafted):
1. Verse 1 melody: immediate Bb→D falling 6th hook, then silence, then F→A answer
2. Chorus dynamic contrast: pad vel 42→65, drums vel 50→75 (verse→chorus)
3. Bass passing tones: chromatic approach notes on beat 4 every 2nd bar
4. Instrumental lead motif: 4 appearances only, each one octave higher (D3→D6)
5. Verse 3 melody: "fall and recovery" arc — D5 down to D4, silence, back to D5
6. Clean guitar removed from Verse 1 — enters at Chorus 1 (bar 24)
7. Verse 2 drums: ghost kick (vel=25) on &2 and ghost snare (vel=20) on &4 every 2nd bar
"""
...
# v4 FIX #7: Verse 2 ghost notes — ghost kick on &2 (vel=25), ghost snare on &4 (vel=20)
# ============================================================
def drums():
    """Drums — sparse, enter at chorus.
    v4: chorus vel boosted to 75. Verse 2 gets ghost notes for pocket."""
    KICK = 36
    SNARE = 38
    HH_CLOSED = 42
    HH_OPEN = 46
    ...
    def verse2_pattern(bar, vel_base, section_start):
        """v4 FIX #7: adds ghost kick on &2 and ghost snare on &4 every 2nd bar."""
        t = bt(bar)
        ...
        # v4 FIX #7: ghost notes every 2nd bar — barely-there hits for "pocket"
        if bar_in_section % 2 == 1:
            # Ghost kick on the "and" of 2 (vel=25) — very quiet
            E.note(9, KICK, 25, t + QUARTER + EIGHTH, EIGHTH, time_amount=8, vel_amount=2)
            # Ghost snare on the "and" of 4 (vel=20) — even quieter
...
def vocal_melody():
    ...
    # then rises back to D5 (bars 5-8). A complete emotional arc.
    v3_start = V3[0]
    ...
    # Bars 7-8: A4 to D5 — return home. Full circle.
    t = bt(v3_start + 6)
    E.note(8, N('A',4), 38, t, WHOLE, time_amount=5, vel_amount=2)
    t = bt(v3_start + 7)
    E.note(8, N('D',5), 30, t, WHOLE, time_amount=5, vel_amount=2)  # naked D5 — the final note


# ============================================================
# SYNTH LEAD — D-F-A RISING MOTIF (Ch 10) — the "glow"
# v4 FIX #4: OCTAVE CLIMBING — only 4 appearances, each one octave higher.
# The RARITY makes it special. 3 notes per appearance, not 64 repetitions.
# ============================================================
def synth_lead_rising_motif():
    ...
    # v4 FIX #4: 4 appearances, each one octave higher
    # Bar 58 (bar 0 of INSTR): D3-F3-A3 — buried, distant
    # Bar 62 (bar 4): D4-F4-A4 — emerging
    # Bar 66 (bar 8): D5-F5-A5 — present, bright
    # Bar 70 (bar 12): D6-F6-A6 — soaring, climax

    appearances = [
        (58, 3, 30),   # bar 58: octave 3, vel 30 (buried, distant)
        (62, 4, 50),   # bar 62: octave 4, vel 50 (emerging)
        (66, 5, 75),   # bar 66: octave 5, vel 75 (present, bright)
        (70, 6, 100),  # bar 70: octave 6, vel 100 (soaring, climax)
    ]

    for bar, octave, vel_base in appearances:
        t = bt(bar)
