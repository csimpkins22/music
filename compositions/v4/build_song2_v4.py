#!/usr/bin/env python3
"""
Song 2 v4: "The Machinery of Sunlight"
Key: F major (modulates to Gb for final chorus) | Tempo: 92 BPM | 4/4 | ~3:50

v4 songwriter notes:
Building on v3's solid structure with targeted improvements to melody, harmony,
rhythm, and arrangement. Every change earns its place.

MELODY CHANGES:
- Verse melody rebuilt around intervallic 5ths (F5→C6, A5→D5) for hook identity
- Chorus melody uses held-note approach: C6 held a full bar, then rapid 16th burst
- Verse and chorus now have distinct rhythmic DNA (leaps vs. sustain-to-burst)

HARMONIC CHANGES:
- Verse 2nd 8-bar cycle: bar 3 Bb replaced with Gm (vi chord, adds melancholy)
- Pre-chorus bass walks chromatically: A→Bb→B→C matching vocal rise
- Modulation prep: Db7 chord (V7 of Gb) played solo in piano before final chorus

RHYTHM/ARRANGEMENT CHANGES:
- Verse drums build progressively over 4×4-bar blocks (kick only → ghost snare → hi-hat 8ths → full)
- Bass adds chromatic walk on beat 4 of every 4th verse bar
- One-beat silence (beat 4 of bar 23) before chorus crash — micro-silence for impact
"""
...
Gm4 = make_chord(N('G',4), 'minor')  # NEW: vi chord for verse reharmonization
...
Db4_7 = make_chord(N('Db',4), '7')    # NEW: V7 of Gb for modulation prep
...
def driving_bass(bar_start, root, vel=58, bar_num=0, next_root=None, chromatic_walk_beat4=False):
    ...
    if bar_num % 8 == 7 and next_root is not None:
        ...
        # FIX 4: Every 4th verse bar — beat 4 chromatic walk to next root
        fifth = root + 7
        seq = [root, root, fifth, root, root, fifth]  # beats 1-3 normal
        # Beat 4: two 8th notes walking chromatically toward next root
        step = 1 if next_root > root else -1
        walk_start = root + step  # one chromatic step from root toward next root
...
def chromatic_bass_walk(bar_start, notes_sequence, vel=58, bar_num=0):
    """Play a chromatic bass walk: one note per beat across a bar.
    notes_sequence: list of 4 MIDI notes, one per beat."""
    bvel = breathing_vel(vel, bar_num, 8, 4)
    for i, note in enumerate(notes_sequence):
        v = bvel + (4 if i == 0 else 0) + random.randint(-3, 3)
        # Two 8ths per beat for driving feel
...
def full_drums(bar_start, pattern='verse', bar_num=0, verse_build_phase=None):
    """Full drum kit.
    verse_build_phase: 0-3 for progressive verse build (FIX 3).
      0 = kick on 1 only
    ...
    if pattern == 'verse' and verse_build_phase is not None:
        # FIX 3: Progressive verse drum build
        if verse_build_phase == 0:
            # Phase 0: kick on beat 1 only — breathing space
            E.note(ch, kick, bvel_k, bar_start, QUARTER, time_amount=5, vel_amount=5)

        elif verse_build_phase == 1:
            # Phase 1: kick on 1 + snare ghost notes on beat 2+
            E.note(ch, kick, bvel_k, bar_start, QUARTER, time_amount=5, vel_amount=5)
            # Ghost snare on beat 2, beat 3+, beat 4
            ...
            # Phase 2: kick on 1 + ghost snares + hi-hat 8ths
            E.note(ch, kick, bvel_k, bar_start, QUARTER, time_amount=5, vel_amount=5)
            E.note(ch, snare, 25, bar_start + QUARTER + swing_offset(1, 8), EIGHTH, time_amount=10, vel_amount=3)
            E.note(ch, snare, 22, bar_start + HALF + EIGHTH + swing_offset(1, 8), EIGHTH, time_amount=12, vel_amount=3)
            E.note(ch, snare, 28, bar_start + HALF + QUARTER + swing_offset(1, 8), EIGHTH, time_amount=10, vel_amount=3)
            ...
            # Phase 3: full verse pattern (same as original)
            E.note(ch, kick, bvel_k, bar_start, QUARTER, time_amount=5, vel_amount=5)
        ...
        # Fill that stops after beat 3 — leaves beat 4 silent for the breath (FIX 6)
        E.note(ch, kick, bvel_k + 8, bar_start, QUARTER, time_amount=4)
        E.note(ch, snare, 45, bar_start + QUARTER, EIGHTH, time_amount=8)
        E.note(ch, snare, 50, bar_start + QUARTER + EIGHTH, EIGHTH, time_amount=6)
        E.note(ch, snare, 55, bar_start + HALF, EIGHTH, time_amount=5)
...
# --- VERSE 1 (16 bars) ---
# FIX 3: Progressive drum build over 4×4-bar blocks
for i in range(16):
    ci = (i // 2) % 4
    next_ci = ((i + 1) // 2) % 4

    # FIX 2: Reharmonize 2nd 8-bar cycle (bars 8-15 within verse)
    ...
    # FIX 4: Every 4th verse bar, chromatic bass walk on beat 4
    use_chromatic = (i % 4 == 3)
    driving_bass(bt(b + i), bass_root, vel=53, bar_num=i,
                 next_root=next_bass, chromatic_walk_beat4=use_chromatic)

    # FIX 3: Determine drum build phase (0-3) based on 4-bar block
...
# FIX 1: Verse 1 melody with intervallic 5ths
v1_mel = [
    ...
    # Phrase 4: resolution — 5th from A5 to D5, then settle on F5
    [(N('A',5), QUARTER), (N('D',5), HALF+EIGHTH), (N('E',5), EIGHTH),
...
for i in range(4):
    ...
    if i < 3:
        ...
        # FIX 6: Bar 23 (last pre-chorus bar) — fill stops at beat 3, beat 4 is SILENCE
        full_drums(bt(b + i), 'fill_beat3', bar_num=19)
        # FIX 4: Pre-chorus chromatic bass walk A→Bb→B→C
        chromatic_bass_walk(bt(b + i), [N('A',2), N('Bb',2), N('B',2), N('C',3)],
                           vel=56, bar_num=19)
        # The bass walk ends on beat 3; beat 4 is silent (no notes scheduled for beat 4)
...
# FIX 5: Chorus melody — HELD NOTE approach
# C6 held for full bar (tension), resolve to Bb5, then rapid 16th burst
ch_mel = [
    # Bar 1-2: C6 HELD for a full bar, then Bb5 resolution, then rapid 16th burst A5
    [(N('C',6), WHOLE),
     (N('Bb',5), HALF), (N('A',5), SIXTEENTH), (N('Bb',5), SIXTEENTH),
     (N('A',5), SIXTEENTH), (N('G',5), SIXTEENTH), (N('A',5), QUARTER+EIGHTH), (N('F',5), EIGHTH)],
    # Bar 3-4: held Bb5, then burst
    [(N('Bb',5), HALF+QUARTER), (N('A',5), QUARTER),
     (N('F',5), HALF), (None, HALF)],
    # Bar 5-6: held C6 again (callback), burst resolves differently
    [(N('C',6), WHOLE),
     (N('D',6), QUARTER), (N('C',6), SIXTEENTH), (N('Bb',5), SIXTEENTH),
     (N('A',5), SIXTEENTH), (N('G',5), SIXTEENTH), (N('A',5), HALF)],
    # Bar 7-8: resolution phrase
...
for i in range(16):
    ...
    # FIX 2: Reharmonize 2nd 8-bar cycle
    ch, bass_root = get_verse_chord_and_bass(i, ci)
    next_ch, next_bass = get_verse_chord_and_bass(i + 1 if i < 15 else 0, next_ci)
    ...
    # FIX 3: Progressive drum build for verse 2 as well
    verse_build = i // 4
...
for i in range(4):
    ...
    if i < 3:
        ...
        # FIX 6: Beat 4 silence before chorus 2
...
# FIX 7: Instead of just a pad swell, play Db7 (V7 of Gb) on piano alone
# This EARNS the modulation harmonically
# Piano plays Db7 chord — the dominant 7th that resolves to Gb
for note in Db4_7:
...
ch_mel_gb = [
    # Held Db6 for a full bar, burst resolves
    [(N('Db',6), WHOLE),
...
build_midi(E, 'The Machinery of Sunlight', bpm=92, total_bars=85,
           ...
           output_path='/home/user/music/compositions/v4/02_the_machinery_of_sunlight.mid')
