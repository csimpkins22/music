#!/usr/bin/env python3
"""
Song 1 v4: "Dial Tone Lullaby"
Key: C major | Tempo: 72 BPM | 4/4 | ~4:30

v4 songwriter notes:
Building on v3's harmonic foundations (borrowed chords, falling 6th hook).
This version addresses the arrangement and melodic shortcomings:

1. VERSE 1 MELODY LEAPS: Bars 8-11 now open G5→C4 (dramatic drop), climb back.
2. VERSE 2 MELODY ASCENDING: Starts C5, climbs E5→G5→A5 — opposite contour to v1.
   The falling 6th hook reappears as A5→C#4 (minor 6th — darker).
3. BASS CHROMATIC WALKS: Verse 1 bars 8-11 get quarter-note chromatic approaches
   (C-B-A, A-G-F, F-F#-G) instead of static whole notes.
4. SYNTH PAD INVERSIONS: In chorus, pad plays first inversions against piano root
   position — voices finally separate.
5. BRIDGE PIANO CHROMATIC VOICE LEADING: Flowing eighth-note arpeggios with
   chromatic top-note walks (A→Ab, D→Db, Eb→D) connecting borrowed chords.
6. DRUMS BUILD IN VERSE 2: Minimal→verse→energetic→fill across 16 bars,
   not identical 'verse' pattern throughout.
7. CHORUS 1 COUNTER-MELODY: True contrary motion — synth ascends when vocal
   descends, plays moving sixteenths against held vocal notes.
"""
...
def piano_bridge_chromatic(bar_start, chord_notes, chromatic_top_from, chromatic_top_to, vel=43, bar_num=0):
    """FIX #5: Bridge piano with flowing eighth-note arpeggios and chromatic
...
def bass(bar_start, root, vel=48, bar_num=0, pattern='whole', next_root=None):
    ...
    if pattern == 'whole':
        ...
        # FIX #3: Chromatic walking bass — plays quarter-note walk pattern
        # walk_notes is a list of 4 quarter-note pitches for the bar
        # (caller sets this via the 'walk_notes' parameter, passed through next_root
        #  but we handle it in the verse 1 section directly)
        E.note(6, root, bvel, bar_start, BAR4 - 40, time_amount=10, vel_amount=4)


def bass_chromatic_walk(bar_start, walk_notes, vel=38, bar_num=0):
    """FIX #3: Bass plays explicit quarter-note chromatic walk pattern."""
    bvel = breathing_vel(vel, bar_num, 8, 4)
...
def drums(bar_start, pattern='minimal', bar_num=0):
    ...
    if pattern == 'minimal':
        ...
        # FIX #6: 'verse' pattern with added snare on "and" of 3 for more drive
        E.note(ch, kick, bvel_k, bar_start, QUARTER, time_amount=5, vel_amount=4)
        E.note(ch, snare, h_vel(28, 4), bar_start + QUARTER, QUARTER, time_amount=8, vel_amount=5)
        E.note(ch, kick, bvel_k - 5, bar_start + HALF, QUARTER, time_amount=6, vel_amount=4)
        E.note(ch, snare, h_vel(28, 4), bar_start + HALF + QUARTER, QUARTER, time_amount=8, vel_amount=5)
...
for i in range(16):
    ...
    # Bass enters bar 8
    # FIX #3: Bars 8-11 get chromatic walking bass instead of whole notes
    if i >= 8:
        if i == 8:
...
# FIX #1: VERSE 1 MELODY — dramatic leap in bars 8-11
# Opens G5 then JUMPS DOWN to C4 (octave+5th), climbs back through E→G
# The signature falling 6th (G→B3) still appears in phrase 3
v1_phrases = [
    # Phrase 1 (bars 8-9): G5 then dramatic DROP to C4, climb to E4
    # G5=79, C4=60 — that's a huge leap that grabs attention
...
for i in range(8):
    ...
    if i >= 6:
        ...
        bass_root = N('Ab',1)
        pad_voicing = AB_PAD_INV[0]  # FIX #4: pad inversion for Ab
    else:
        voicing = gv(CHORUS_CHORDS, ci, i)
        bass_root = CHORUS_BASS[ci]
        pad_voicing = gv(CHORUS_PAD_INVERSIONS, ci, i)  # FIX #4: pad inversions

    piano_arpeggio(bt(b + i), voicing, vel=55, bar_num=i + 8)

    # FIX #4: Pad plays INVERSIONS, not same voicing as piano
    if i % 2 == 0:
...
# FIX #7: TRUE counter-melody in chorus 1 — contrary motion counterpoint
# When vocal descends (C→E falling 6th), synth ASCENDS (E→G→B)
# When vocal holds long notes, synth plays moving sixteenths
lead_ch1 = [
    # Phrase 1: Vocal descends C→E (falling 6th). Synth RISES E4→G4→B4→C5 (contrary!)
    # Then when vocal holds on G, synth plays moving sixteenths
    ...
    # When vocal holds A, synth does a little turn
...
# FIX #6: Drums build through verse 2 instead of identical 'verse' pattern
for i in range(16):
    is_second_half = i >= 8
    chords = VERSE_CHORDS_B if is_second_half else VERSE_CHORDS_A
    bass_notes = VERSE_BASS_B if is_second_half else VERSE_BASS_A
    ...
    # FIX #6: Drums build across verse 2
    # Bars 0-7 (absolute 32-39): 'minimal'
    # Bars 8-11 (absolute 40-43): 'verse'
    # Bars 12-14 (absolute 44-46): 'verse_energetic' (added snare on "and" of 3)
...
# FIX #2: Verse 2 melody — ASCENDING where verse 1 descended
# Starts on C5, climbs to E5→G5→A5 (higher than verse 1 ever reached)
# The falling 6th hook reappears as A5→C#4 (minor 6th — darker for verse 2)
v2_phrases = [
    ...
    # Phrase 3 (bars 36-37): THE DARK HOOK — A5 drops to C#4 (minor 6th — darker than v1's major 6th)
...
for i in range(8):
    ...
    if i >= 6:
        voicing = gv(AB_CHORD, 0, i)
        bass_root = N('Ab',1)
        pad_voicing = AB_PAD_INV[0]  # FIX #4
    else:
        voicing = gv(CHORUS_CHORDS, ci, i + 8)
        bass_root = CHORUS_BASS[ci]
        pad_voicing = gv(CHORUS_PAD_INVERSIONS, ci, i)  # FIX #4

    piano_arpeggio(bt(b + i), voicing, vel=57, bar_num=i + 32)

    # FIX #4: Pad plays inversions
    if i % 2 == 0:
...
# FIX #5: Piano uses flowing eighth-note patterns with chromatic voice-leading
# The top note of each chord walks chromatically to the next on beat 4:
#   Dm: top A → Ab (chromatic approach down to Ab chord)
#   Bb: top D → Db (chromatic approach down)
#   Ab: top Eb → D (chromatic approach down to G chord's D)
#   G: resolves naturally

# Chromatic top-note targets for each 2-bar chord block:
# Dm (A4=69): walks A→Ab (69→68) on beat 4 of bar 2 → approaching Ab chord
# Bb (D4=62→F4=65): walks D→Db (62→61) — but the top of Bb voicing is F4
#   Actually Bb voicing = Bb3-D4-F4, top = F4(65), so walk to... let's use D5 range
#   Bb voicing [Bb3, D4, F4]: top note in arpeggio reaches F4+12=F5=77 or D4=62
#   Let's be precise: the chord arpeggio top note pattern varies. We'll set explicit top notes.

for i in range(8):
    ci = (i // 2) % 4
    voicing = gv(BRIDGE_CHORDS, ci, i)

    # Determine chromatic top-note walk for this bar
    if ci == 0:  # Dm: top note A4, walks to Ab4
        ...
        # Top note of Bb arp = D4 (in higher octave context), walk D5→Db5
...
build_midi(E, 'Dial Tone Lullaby', bpm=72, total_bars=86,
           ...
           output_path='/home/user/music/compositions/v4/01_dial_tone_lullaby.mid')
