#!/usr/bin/env python3
"""
Song 3 v4: "Beautiful Machines"
Key: Eb major | Tempo: 84 BPM | 4/4 | ~124 bars

v4 handcrafted changes — 7 specific improvements over v3:

1. VERSE 2 MELODY LEAPS: Melody built on 4ths (Eb5→Ab5→Db6) instead of
   stepwise Eb scale. The Db6 foreshadows the chorus's borrowed chord.

2. VERSE 1 POLYPHONY: Piano call-and-response after each vocal phrase.
   Vocal ends on Bb → piano answers Eb-G ascending.
   Vocal ends on G → piano answers Bb-Ab descending.

3. CHORUS BASS CHROMATIC WALKS: Fm bar = F→Eb→Db descending walk,
   then jump to Db for the Db bar. Chromatic connection earns the
   borrowed chord instead of imposing it.

4. VERSE 2 DRUMS RHYTHMIC SHIFTS: Every 4-bar group gets:
   Bar 1: kick on "and" of 2 (syncopation)
   Bar 3: hi-hat drops for one beat (breathing room)
   Bar 4: ghost snare on beat 4+ (anticipation)

5. CRESCENDO KICK DISPLACEMENT: Kick shifts forward by one eighth
   every 4 bars: beat 1 → 1+ → "and" of 1 → beat 2.
   Creates "time stretching" tension.

6. VERSE 3 COUNTER-MELODY INTEGRATION: Pad chord includes Db when
   counter-melody hits Db, creating temporary Ebsus4-like harmony
   that acknowledges the counter-melody.

7. PIANO-VOCAL CHORUS SPOTLIGHT: When vocal hits C6 (chorus peak),
   piano stops its arpeggio and plays a single octave C4-C5,
   clearing texture for the climax.
...
def drums(bar_start, pattern='sparse', bar_num=0, group_pos=None):
...
build_midi(E, 'Beautiful Machines', bpm=84, total_bars=124,
           output_path='/home/user/music/compositions/v4/03_beautiful_machines.mid')
