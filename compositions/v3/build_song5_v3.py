...
v3 changes (handcrafted):
- Structure: Intro(8) V1(16) CH1(8) V2(16) CH2(8) SILENCE(2) INSTR(16) V3(8) OUTRO(8) = 90 bars
- Chorus harmony: Bb-F-A7-Dm (A7 replaces C — dominant V chord with raised 7th C#)
- Instrumental: Dm-Dm/C-Bb-Gm/Bb-A(extended 2 bars) — chromatic bass descent D-C-Bb-Bb-A
- Verse 1 opens with held whole-note D5
- Chorus vocal sings C# on A7 chord, resolving to D — harmonic minor magic
- Instrumental: new D-F-A rising motif on synth lead, starts quiet, becomes loudest
- 2 bars of silence (organ drone only) between CH2 and INSTR
- Organ drone adds C natural for Dm7 tension; F# over A major in instrumental
- Clean guitar plays every OTHER bar in V1
- Distorted guitar plays single-note melody bars 9-12 of instrumental
- Outro: last 4 bars organ only, pitch wobble to +/-400 cents, fading to nothing
- Verse 3: completely new vocal melody — sparse, spoken-word rhythm
"""
...
# The 5-chord pattern over 5 bars, but A gets 2 bars.
# We'll handle this with a custom 5-bar cycle mapped to 16 bars.
INSTR_VOICINGS_5 = [
    # Dm
...
# v3: In verse 1, play every OTHER bar. Spaces are as important as notes.
# ============================================================
def clean_guitar():
    """Clean guitar — sparse, ringing chords. v3: every other bar in V1."""
    ...
    # Verse 1 — v3: every OTHER bar only. Let the spaces breathe.
    for bar in range(V1[0], V1[1]):
        if (bar - V1[0]) % 2 == 0:  # play bars 0, 2, 4, 6... of verse
            chord_idx = (bar - V1[0]) % 4
            chord = get_voicing(VERSE_VOICINGS, chord_idx, bar)
            guitar_bar(bar, chord, 48, bar)

    # Chorus 1 — every bar, slightly fuller
...
# ============================================================
def drums():
    """Drums — sparse, enter at chorus. v3: crash into instrumental after silence."""
    KICK = 36
    SNARE = 38
    HH_CLOSED = 42
    HH_OPEN = 46
...
# v3 changes:
#   - V1 opens with LONG held D5 (whole note)
#   - Chorus: C# on A7 chord, resolving to D — harmonic minor leading tone
#   - V3: COMPLETELY new melody — sparse, spoken-word rhythm
# ============================================================
def vocal_melody():
    """Vocal melody — v3: held opening D5, C# on A7 resolution, new V3 melody."""
    ...
    # === VERSE 1 ===
    # v3: Opens with a LONG held D5 (whole note). Just one note. Let atmosphere establish.
    # Then the melody unfolds from there.
    phrase(8, [
        (0, 0, WHOLE),                    # D — HELD. The atmosphere establishes.
        # (silence for bar 2 — the D rings out)
        (2, BAR4 * 2, HALF),              # F — melody finally moves
        (3, BAR4 * 2 + HALF, QUARTER),    # G
        (4, BAR4 * 3, HALF),              # A
        (3, BAR4 * 3 + HALF, HALF),       # G — settling
    ], 55)
    ...
    # === VERSE 3 SPARSE ===
    # v3: COMPLETELY new melody. Not a variation — a new melody.
    # Sparse, almost spoken-word rhythm. Short phrases with long silences.
    # This is the song's coda, its final thought.
    v3_start = V3[0]

    # Phrase 1: Two short notes, then silence. Like a whispered observation.
    t = bt(v3_start)
    E.note(8, N('A',4), 40, t, QUARTER, time_amount=6, vel_amount=3)              # A4 — low, quiet
    E.note(8, N('F',4), 38, t + HALF, QUARTER + EIGHTH, time_amount=6, vel_amount=3)  # F4 — descending
    # (long silence: rest of bar 0, all of bar 1)

    # Phrase 2: bar 2 — a single sustained note, different register entirely
    t = bt(v3_start + 2)
    E.note(8, N('Bb',4), 42, t + QUARTER, DOTTED_HALF, time_amount=6, vel_amount=3)  # Bb4 — unexpected

    # (silence: bar 3)

    # Phrase 3: bar 4-5 — three notes, rhythm like speech: "and then... gone"
    t = bt(v3_start + 4)
    E.note(8, N('G',4), 38, t, QUARTER, time_amount=6, vel_amount=3)               # G4
    E.note(8, N('E',4), 35, t + HALF + QUARTER, QUARTER, time_amount=6, vel_amount=3)  # E4 — the lowest we go
    t = bt(v3_start + 5)
    E.note(8, N('D',4), 33, t + QUARTER, HALF, time_amount=6, vel_amount=3)        # D4 — resolution, very quiet

    # (silence: bar 6)

    # Phrase 4: bar 7 — final note. Just one. The last thing you hear before the outro.
    t = bt(v3_start + 7)
    E.note(8, N('A',4), 30, t, WHOLE, time_amount=5, vel_amount=2)                 # A4 — open, unresolved


# ============================================================
# SYNTH LEAD — D-F-A RISING MOTIF (Ch 10) — the "glow"
# v3: New channel for the rising motif during instrumental.
# Starts very quiet, gradually becomes the loudest thing in the mix.
# The "peripheral glow" — light breaking through.
# ============================================================
def synth_lead_rising_motif():
    """The 'glow' — D-F-A rising 3-note figure, starts quiet, becomes loudest."""

    # The motif: D5-F5-A5 (rising minor triad arpeggio — the glow breaking through)
    motif_notes = [N('D',5), N('F',5), N('A',5)]

    for bar in range(INSTR[0], INSTR[1]):
        t = bt(bar)
        bar_in_section = bar - INSTR[0]

        # Dynamic arc: starts at vel 20, ends at vel 100+
        # This is THE moment — the peripheral glow becoming the brightest thing
        progress = bar_in_section / 15.0  # 0.0 to 1.0
        # Exponential growth curve — barely there at start, overwhelming at end
        vel = int(18 + 85 * (progress ** 1.8))
        vel = min(110, vel)
        vel = breathing_vel(vel, bar, cycle_bars=4, depth=3)

        # Play the 3-note motif repeating through the bar
        # Each repetition on quarter note boundaries: 4 repetitions per bar
        for rep in range(4):
            rep_start = t + rep * QUARTER
            for i, note in enumerate(motif_notes):
                note_t = rep_start + i * (QUARTER // 3)  # triplet feel within the quarter
                dur = QUARTER // 3 - 15
                # Slight accent on the A (highest note — the peak of the glow)
    ...
    t = bt(V3[0])
    for i, note in enumerate(motif_notes):
        E.note(10, note, 30, t + i * (QUARTER // 3), QUARTER, time_amount=6, vel_amount=3)
