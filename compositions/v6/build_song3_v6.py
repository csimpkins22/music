#!/usr/bin/env python3
"""
Song 3 v6: "Beautiful Machines"
Key: Eb major | Tempo: 84 BPM | 4/4 | ~124 bars

v6 production-level improvements — 3 specific fixes over v5:

1. VERSE 3 SYNTH DROP (bars 64-67): When synth arp drops out, drums now
   also thin to 'sparse' pattern for those 4 bars. Ghost snare hits on
   "and" of beat 2 (vel=12) fill the space subtly. Both arp AND drums
   pull back together, creating a true acoustic moment. Bars 68-69:
   both return with energy.

2. CHORUS PIANO VOICE-LEADING (bars 48-59): On the Db chord (chord index 1),
   a grace-note approach in the piano top voice: E→Eb (two sixteenth notes)
   leading into the Db chord tone. Mirrors the bass chromatic ascent and
   creates harmonic unity.

3. CRESCENDO SNARE SAFETY (bars 98-101): Check if syncopated snare hits
   collide with displaced kick. If kick_displacement >= 2, move the extra
   snare hit to beat 3 "and" instead of beat 2 "and". Prevents rhythmic
   collision.
...
build_midi(E, 'Beautiful Machines', bpm=84, total_bars=124,
           output_path='/home/user/music/compositions/v6/03_beautiful_machines.mid')
