#!/usr/bin/env python3
"""
Song 2 v6: "The Machinery of Sunlight"
Key: F major (modulates to Gb for final chorus) | Tempo: 92 BPM | 4/4 | ~3:50

v6 songwriter notes:
Building on v5 with three production-level improvements:

FIX 1: VERSE 2 CLIMAX BUILD (bars 44-47) — clear energy escalation approaching
  chorus 2. Snare velocity increases +3 each bar (44→47). Synth lead re-enters
  bar 46 with short rhythmic eighth-note punctuations. Signals "something is coming."

FIX 2: PRE-CHORUS POWER CHORD SILENCE (bar 51, beat 4) — when synth arp goes
  silent on beat 4 of bar 51, power chords also drop velocity to 38 on that beat.
  Micro-silence respected across ALL instruments — everything breathes together.

FIX 3: CHORUS PIANO VOICE-LEADING (bar 60-61, Db→Fm transition) — grace-note
  chromatic run in piano (sixteenth notes: Db→Eb→E leading to beat 4) mirrors
  the bass chromatic ascent. Piano and bass lock together harmonically.
"""
...
Fm4 = make_chord(N('F',4), 'minor')   # v6: Fm for transition voice-leading
...
def power_chord(bar_start, root, vel=70, pattern='chorus', bar_num=0,
                beat4_vel_override=None):
    """Distorted guitar power chords.
    beat4_vel_override: if set, beat 4 (second half-bar) uses this velocity instead."""
    ...
    if pattern == 'chorus':
        for n in pc:
            ...
            # v6 FIX 2: allow beat 4 velocity override for micro-silence
            if beat4_vel_override is not None:
                v2 = beat4_vel_override + random.randint(-2, 2)
            else:
        ...
        # v6 FIX 2: support beat4_vel_override for sustained pattern too
        if beat4_vel_override is not None:
            # Play beats 1-3 at normal velocity, beat 4 at override
            for n in pc:
                E.note(4, n, bvel + random.randint(-3, 3), bar_start,
                       HALF + QUARTER - 30, time_amount=8, vel_amount=5)
                E.note(4, n, beat4_vel_override + random.randint(-2, 2),
                       bar_start + HALF + QUARTER, QUARTER - 30,
...
def full_drums(bar_start, pattern='verse', bar_num=0, verse_build_phase=None,
    ...
    snare_vel_boost: v6 FIX 1 — extra velocity added to snare hits for climax build.
    """
    ch = 9
    ...
    if pattern == 'verse' and verse_build_phase is not None:
        if verse_build_phase == 0:
            ...
            E.note(ch, kick, bvel_k, bar_start, QUARTER, time_amount=5, vel_amount=5)
            E.note(ch, snare, 25 + snare_vel_boost, bar_start + QUARTER + swing_offset(1, 8), EIGHTH, time_amount=10, vel_amount=3)
            E.note(ch, snare, 22 + snare_vel_boost, bar_start + HALF + EIGHTH + swing_offset(1, 8), EIGHTH, time_amount=12, vel_amount=3)
            E.note(ch, snare, 28 + snare_vel_boost, bar_start + HALF + QUARTER + swing_offset(1, 8), EIGHTH, time_amount=10, vel_amount=3)
            ...
            E.note(ch, kick, bvel_k, bar_start, QUARTER, time_amount=5, vel_amount=5)
            E.note(ch, snare, h_vel(52 + snare_vel_boost, 5), bar_start + QUARTER, QUARTER, time_amount=7, vel_amount=5)
            E.note(ch, kick, bvel_k - 5, bar_start + HALF, QUARTER, time_amount=6, vel_amount=5)
            E.note(ch, snare, h_vel(52 + snare_vel_boost, 5), bar_start + HALF + QUARTER, QUARTER, time_amount=7, vel_amount=5)
            for i in range(8):
                v = 32 + (5 if i % 2 == 0 else 0) + random.randint(-4, 4)
                E.note(ch, hh_c, v, bar_start + i * EIGHTH + swing_offset(i, 8), EIGHTH, time_amount=5)
            if bar_num % 2 == 0:
                E.note(ch, snare, 18 + snare_vel_boost, bar_start + EIGHTH + swing_offset(1, 8), EIGHTH, time_amount=12)
...
for i in range(16):
    ...
    # v6 FIX 1: Bars 44-47 (i=12..15) — snare velocity escalation approaching chorus 2.
    # Snare vel increases +3 each bar from bar 44 (i=12) to bar 47 (i=15).
    snare_boost = 0
    if 12 <= i <= 15:
        snare_boost = (i - 12) * 3  # 0, 3, 6, 9

    full_drums(bt(b + i), 'verse', bar_num=i + 32, verse_build_phase=verse_build,
               snare_vel_boost=snare_boost)

    if i % 4 == 0:
        synth_riff(bt(b + i), vel=50, bar_num=i + 32)

    # v6 FIX 1 (continued): Synth lead re-enters bar 46 (i=14) with short rhythmic
    # eighth-note punctuations — not sustained, just rhythmic stabs signaling buildup.
    if i == 14:
        # Eighth-note punctuations on synth lead: F5, A5, Bb5, C6 — quick rhythmic jabs
        stab_notes = [N('F',5), N('A',5), N('Bb',5), N('C',6),
                      N('A',5), N('Bb',5), N('C',6), N('A',5)]
        for si, sn in enumerate(stab_notes):
            sv = breathing_vel(52 + si, i + 32, 4, 4)
            E.note(2, sn, sv, bt(b + i) + si * EIGHTH + swing_offset(si, 8),
                   EIGHTH - random.randint(30, 50), time_amount=6, vel_amount=4)
    if i == 15:
        # Bar 47: continue eighth-note stabs, ascending to build more
        stab_notes_2 = [N('Bb',5), N('C',6), N('D',6), N('C',6),
                        N('Bb',5), N('C',6), N('D',6), N('F',6)]
        for si, sn in enumerate(stab_notes_2):
            sv = breathing_vel(55 + si, i + 32, 4, 4)
            E.note(2, sn, sv, bt(b + i) + si * EIGHTH + swing_offset(si, 8),
                   EIGHTH - random.randint(30, 50), time_amount=6, vel_amount=4)
...
for i in range(4):
    ...
    if i < 3:
        ...
        # v5 FIX 2 + v6 FIX 2: Bar 51 — synth arp drops out entirely on beat 4.
        # Power chords also drop velocity to 38 on beat 4. Everything breathes together.
        synth_arp(bt(b + i), ch, vel=38, bar_num=i + 48, stop_at_beat=3)
        power_chord(bt(b + i), PRE_BASS[i] + 24, vel=50 + i * 5, pattern='sustained',
                    bar_num=i + 4, beat4_vel_override=38)
...
# v6 FIX 3: CHORUS PIANO VOICE-LEADING (bar 60-61, Db→Fm transition)
# Grace-note chromatic run in piano: sixteenth notes Db→Eb→E leading to beat 4
# of bar 60. This mirrors the bass chromatic ascent from Db up to F.
# The run lands on beat 4, locking with the bass's arrival at F.
piano_chromatic = [N('Db',5), N('Eb',5), N('E',5)]
for ci_p, pn in enumerate(piano_chromatic):
    # Place three sixteenth notes leading INTO beat 4 (beat 4 = 3*QUARTER)
    # So they start at beat 3 + 1 sixteenth
    t_piano = bt(b) + 2 * QUARTER + SIXTEENTH + ci_p * SIXTEENTH
    pv = breathing_vel(50 + ci_p * 3, 60, 4, 4)
    E.note(0, pn, pv, t_piano, SIXTEENTH - random.randint(8, 15), time_amount=6, vel_amount=3)
# Landing note: F on beat 4 — resolves the chromatic run
E.note(0, N('F',5), breathing_vel(58, 60, 4, 4), bt(b) + 3 * QUARTER,
       QUARTER - random.randint(10, 20), time_amount=6, vel_amount=3)
# Also add the chord tones of Fm on beat 4 for harmonic context
for fn in Fm4:
    E.note(0, fn, breathing_vel(44, 60, 4, 4), bt(b) + 3 * QUARTER,
           QUARTER - random.randint(10, 20), time_amount=8, vel_amount=4)

# Bar 61: piano continues with F minor color resolving to F major
for fn in F4:
    E.note(0, fn, breathing_vel(42, 61, 4, 4), bt(b + 1),
           HALF - random.randint(10, 20), time_amount=8, vel_amount=4)
...
build_midi(E, 'The Machinery of Sunlight', bpm=92, total_bars=85,
           ...
           output_path='/home/user/music/compositions/v6/02_the_machinery_of_sunlight.mid')
